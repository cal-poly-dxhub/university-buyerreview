...
def run_data_sec_classification(state: dict) -> dict:
    ...
    prompt = TASK_PROMPT_REGISTRY.get("SECURITY_CLASSIFICATION", "").replace("{doc_text}", doc_text)
...
def goods_services_classification(service_info: dict) -> str:
    ...
    prompt = TASK_PROMPT_REGISTRY.get("GOODS_SERVICES", "").replace("{doc_text}", doc_text)
