...
class DocumentProcessingStack(Stack):
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        # Allow ingest to write job files to documents bucket
        self.documents_bucket.grant_put(ingest_role)
        ...
        ingest_fn = _lambda.Function(
            ...
            memory_size=256,
            environment={
                "REQUESTS_BUCKET": self.documents_bucket.bucket_name,
                "REQUESTS_PREFIX": JOBS_PREFIX,
            },
            log_retention=logs.RetentionDays.ONE_MONTH,
        )

        # ---------------- S3 → Processor trigger on job files ----------------
        self.documents_bucket.add_event_notification(
            s3.EventType.OBJECT_CREATED,
            s3n.LambdaDestination(self.po_workflow_lambda),
            s3.NotificationKeyFilter(prefix=JOBS_PREFIX, suffix=".json"),
        )

        # ---------------- API → Ingest Lambda ----------------
        api = apigw.RestApi(self, "ProcessUrisRestApi", rest_api_name="document-processing", deploy=True)
        process_resource = api.root.add_resource("process")
        process_resource.add_method("POST", apigw.LambdaIntegration(ingest_fn, proxy=True))
